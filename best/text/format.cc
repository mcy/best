...
best::row<size_t, size_t> format_spec::compute_padding(
  ...
  switch (alignment.value_or(default_alignment)) {
    case Left: return {0, padding};
    case Center: return {padding / 2, (padding + 1) / 2};
    case Right: return {padding, 0}; break;
  }
}
...
void formatter::update_indent() {
  ...
  for (size_t i = 0; i < indent_; ++i) { out_->push_lossy(config_.indent); }
}
